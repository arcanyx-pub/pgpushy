...
use crate::literal;
use crate::model::{
    Comment, ForeignKey, Index, Objects, QualifiedName, SchemaName, Table, TypeLike,
};
...
fn closure_for(objects: &Objects, target: &SchemaName) -> BTreeSet<QualifiedName> {
    ...
    while let Some(name) = pending.pop() {
        ...
        // What the member itself needs before it can be created: the types and
        // domains its columns are written in, and what those are built from.
        // §4.5 admits no cross-schema reference but a foreign key, so all of
        // this lives in the member's own schema.
        if let Some(table) = objects.tables.iter().find(|table| table.name == name) {
            pending.extend(table.depends_on.iter().cloned());
        }
        if let Some(kind) = objects.types.iter().find(|kind| kind.name == name) {
            pending.extend(kind.depends_on.iter().cloned());
        }
    }
...
fn for_schema(objects: &Objects, target: &SchemaName) -> Result<String, CoreError> {
    ...
    emit_in_order(
        ...
        type_statements(&objects.types, target, &closure)?,
    );
    emit(
        &mut out,
        "tables",
...
fn emit(out: &mut String, label: &str, mut statements: Vec<Sorted>) {
    ...
    emit_in_order(
        out,
        label,
        statements.into_iter().map(|(_, sql)| sql).collect(),
    );
}

/// Emit statements whose order has already been decided.
fn emit_in_order(out: &mut String, label: &str, statements: Vec<String>) {
...
// ---------------------------------------------------------------------------
// Category 2: types, domains and sequences
// ---------------------------------------------------------------------------

/// The only category that is not internally order-free.
///
/// A domain can be defined over another domain, a composite type can have a
/// domain-typed field, and a domain default can call `nextval` — so no fixed
/// order among the three kinds is correct, and pgschema's own `dump` order is
/// a convention rather than a guarantee. (Its dump of a composite type over a
/// domain does emit the domain first, which is the same conclusion reached the
/// other way round.) The order here is topological, with `(schema, name)`
/// breaking ties so that it stays a function of the source content (§11.3).
fn type_statements(
    types: &[TypeLike],
    target: &SchemaName,
    closure: &BTreeSet<QualifiedName>,
) -> Result<Vec<String>, CoreError> {
    let selected: Vec<&TypeLike> = types
        .iter()
        .filter(|kind| kind.name.schema == *target || closure.contains(&kind.name))
        .collect();

    let mut remaining: BTreeMap<&QualifiedName, &TypeLike> =
        selected.iter().map(|kind| (&kind.name, *kind)).collect();
    let mut emitted: BTreeSet<&QualifiedName> = BTreeSet::new();
    let mut out = Vec::with_capacity(selected.len());

    while !remaining.is_empty() {
        // `remaining` is a BTreeMap, so this scans in `(schema, name)` order
        // and the first ready object wins — that is the tie-break.
        let ready = remaining
            .iter()
            .find(|(_, kind)| {
                kind.depends_on
                    .iter()
                    // A dependency the tree does not define is not something
                    // to wait for; `validate` has already rejected those.
                    .all(|need| !remaining.contains_key(need) || emitted.contains(need))
            })
            .map(|(name, kind)| (*name, *kind));

        let Some((name, kind)) = ready else {
            // Postgres will not create a cycle here, so reaching this means
            // pgpushy built one. Emitting in some arbitrary order would
            // produce a document that cannot execute.
            return Err(CoreError::TypeCycle {
                names: remaining.keys().map(|name| name.to_string()).collect(),
            });
        };

        let owned = kind.name.schema == *target;
        out.push(render(&kind.ast, owned, &kind.name.to_string())?);
        emitted.insert(name);
